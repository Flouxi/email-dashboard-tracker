 ...
 * meta can include { ip, country, userAgent }.
 */
async function logRawOpen(email, meta = {}) {
  const now = new Date().toISOString();
  const entry = {
    at: now,
    ip: meta.ip || '',
  ...
  db.rawopens[email].opens.push(entry);
  writeLocalDb(db);
...
/**
 * Flatten every open event across every recipient into one chronological
 * log, most recent first. Powers the hourly chart, country/domain
 * breakdowns, and the Tracking Logs table.
 */
async function getFlattenedLog() {
  const records = await getRawOpens();
  const events = [];
  for (const r of records) {
    for (const o of r.opens) {
      events.push({ email: r.email, at: o.at, ip: o.ip, country: o.country, userAgent: o.userAgent });
    }
  }
...
/**
 * Wipe all raw-open tracking data (the "Delete Logs" button).
 */
async function clearAll() {
  if (USE_KV) {
    const emails = [...new Set(await kv.zrange('rawopen:index', 0, -1))];
    await Promise.all(emails.map((e) => kv.del(`rawopen:${e}`)));
    await kv.del('rawopen:index');
    return;
  }
  const db = readLocalDb();
  db.rawopens = {};
...
module.exports = { logEvent, getAll, getOne, logRawOpen, getRawOpens, getFlattenedLog, clearAll, USE_KV };
